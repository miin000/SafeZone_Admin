...
function ReportCard({ 
  ...
  const priorityConfig = priorityOptions[report.priority as keyof typeof priorityOptions] 
    ?? { color: '#9e9e9e', label: 'Không xác định', icon: '⚪' };
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ display: 'flex', gap: 16, marginBottom: 12, flexWrap: 'wrap' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, opacity: 0.7 }}>
                ...
                <span style={{ fontSize: 13 }}>
                  {report.region_name || report.address || `${report.lat?.toFixed(4)}, ${report.lon?.toFixed(4)}`}
                </span>
              </div>
            ...
            <div style={{ 
              ...
              <div style={{ flex: 1, minWidth: 140 }}>
                <div style={{ fontSize: 10, opacity: 0.5, marginBottom: 4 }}>🎯 Vị trí ca bệnh</div>
              ...
              {report.reporter_lat && report.reporter_lon && (
                <div style={{ flex: 1, minWidth: 140 }}>
                  <div style={{ fontSize: 10, opacity: 0.5, marginBottom: 4 }}>📱 Vị trí người báo cáo</div>
                  <div style={{ fontSize: 12, fontFamily: 'monospace' }}>
                    {report.reporter_lat.toFixed(6)}, {report.reporter_lon.toFixed(6)}
                  </div>
                </div>
              )}
              {report.reporter_lat && report.reporter_lon && report.lat && report.lon && (
                <div style={{ flex: 1, minWidth: 100 }}>
                  <div style={{ fontSize: 10, opacity: 0.5, marginBottom: 4 }}>📏 Khoảng cách</div>
                  <div style={{ fontSize: 12 }}>
                    {calculateDistance(report.lat, report.lon, report.reporter_lat, report.reporter_lon)} km
                  </div>
                </div>
              )}
            </div>
        ...
        <div style={{ padding: 20, maxHeight: '70vh', overflow: 'auto' }}>
          ...
          <div style={{ 
            ...
            <div style={{ 
              ...
              <div style={{ fontWeight: 600, marginBottom: 8, fontSize: 12 }}>📍 Thông tin vị trí</div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, fontSize: 12 }}>
                <div>
                  <div style={{ opacity: 0.5, marginBottom: 4 }}>🎯 Vị trí ca bệnh:</div>
                ...
                {report.reporter_lat && report.reporter_lon ? (
                  <div>
                    <div style={{ opacity: 0.5, marginBottom: 4 }}>📱 Vị trí người báo cáo:</div>
                    <div style={{ fontFamily: 'monospace' }}>
                      {report.reporter_lat.toFixed(6)}, {report.reporter_lon.toFixed(6)}
                    </div>
                  </div>
                  ...
                  <div>
                    <div style={{ opacity: 0.5, marginBottom: 4 }}>📱 Vị trí người báo cáo:</div>
                    <div style={{ color: '#666', fontStyle: 'italic' }}>Không có</div>
                  </div>
                )}
              </div>
              {report.reporter_lat && report.reporter_lon && report.lat && report.lon && (
                <div style={{ marginTop: 8, fontSize: 11, opacity: 0.7 }}>
                  📏 Khoảng cách giữa vị trí ca bệnh và người báo cáo: {' '}
                  <strong>{calculateDistance(report.lat, report.lon, report.reporter_lat, report.reporter_lon)}</strong>
                </div>
              )}
            </div>
            ...
            {report.symptoms && report.symptoms.length > 0 && (
              <div style={{ marginTop: 12 }}>
                <span style={{ opacity: 0.6 }}>Triệu chứng:</span>
...
function calculateDistance(lat1: number, lon1: number, lat2: number, lon2: number): string {
  ...
  const distance = R * c;
  return distance < 1 ? (distance * 1000).toFixed(0) + ' m' : distance.toFixed(2);
}
